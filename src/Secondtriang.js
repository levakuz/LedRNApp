...
import { TouchableOpacity, View, Image, StyleSheet,Alert  , InteractionManager, I18nManager, Button, Text } from 'react-native'
...
export class TriangleNewColorPicker extends React.PureComponent {
  ...
  constructor(props, ctx) {
    ...
    this._onColorSelectedAR = this._onColorSelectedAR.bind(this)
    this._onColorSelected1 = this._onColorSelected1.bind(this)
    this._onColorSelected2 = this._onColorSelected2.bind(this)
    this._onColorSelected3 = this._onColorSelected3.bind(this)
    this._onColorSelected4 = this._onColorSelected4.bind(this)
  ...
  _onColorSelectedAR() {
    const { onColorSelectedAR } = this.props
    const color = tinycolor(this._getColor()).toHexString()
    const room = 'AR'
    onColorSelectedAR && onColorSelectedAR(color, room)
  }
  _onColorSelected1() {
    const { onColorSelected1 } = this.props
    const color = tinycolor(this._getColor()).toHexString()
    const room = '1'
    onColorSelected1 && onColorSelected1(color, room)
  }
  _onColorSelected2() {
    const { onColorSelected2 } = this.props
    const color = tinycolor(this._getColor()).toHexString()
    const room = '2'
    onColorSelected2 && onColorSelected2(color, room)
  }
  _onColorSelected3() {
    const { onColorSelected3 } = this.props
    const color = tinycolor(this._getColor()).toHexString()
    const room = '3'
    onColorSelected3 && onColorSelected3(color, room)
  }
  _onColorSelected4() {
    const { onColorSelected4 } = this.props
    const color = tinycolor(this._getColor()).toHexString()
    const room = '4'
    onColorSelected4 && onColorSelected4(color, room)
  }
  ...
  render() {
    ...
    return (
      <View style={style}>
        ...
        <View style={[styles.colorPreviewsAllRoom, computed.colorPreviews]}>
          
          <TouchableOpacity
            style={[styles.colorPreview, { backgroundColor: selectedColor }]}
            onPress={() => Alert.alert(
              "Выбери режим",
              "1 - Переключение цветов в реальном времени\n2- Переключение сразу",
              [
                {
                  text: "1",
                  onPress: () => this._onColorChange({color}),
                  style: "cancel"
                },
                { text: "2", onPress: () => this._onColorSelectedAR }
              ],
              { cancelable: false }
            )}
            activeOpacity={0.5}>
            <Text style={styles.colorPreviewsText}>Вся комната</Text>
          </TouchableOpacity>
          
          
        </View>
        <View style={[styles.colorPreviews1, computed.colorPreviews]}>
          
          <TouchableOpacity
            style={[styles.colorPreview, { backgroundColor: selectedColor }]}
            onPress={this._onColorSelected1}
            activeOpacity={0.5}>
            <Text style={styles.colorPreviewsText}>ZONE 1</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.colorPreview, { backgroundColor: selectedColor }]}
            onPress={this._onColorSelected2}
            activeOpacity={0.5}>
            <Text style={styles.colorPreviewsText}>ZONE 2</Text>
          </TouchableOpacity>
          
        </View>
        <View style={[styles.colorPreviews2, computed.colorPreviews]}>
          
          <TouchableOpacity
            style={[styles.colorPreview, { backgroundColor: selectedColor }]}
            onPress={this._onColorSelected3}
            activeOpacity={0.5}>
            <Text style={styles.colorPreviewsText}>ZONE 3</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.colorPreview, { backgroundColor: selectedColor }]}
            onPress={this._onColorSelected4}
            activeOpacity={0.5}>
            <Text style={styles.colorPreviewsText}>ZONE 4</Text>
          </TouchableOpacity>
